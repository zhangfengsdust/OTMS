...
$(function(){
	function show(data){
		...
		$.getJSON(_url,_args,function(_data){
			...
			$("tbody tr:last td:eq(11)").find("a").bind("click",function(){
				...
				var flag = confirm("确定要删除'"+name+"'管理员吗？");
				if(flag == false){
					return false;
				}
				var url = "${pageContext.request.contextPath}/member.do";
				var args = {"id":$(this).attr("id"),"time":new Date(),"action":"deleteAdministrator"};
	...
	$("#btn1").click(function(){
		...
		var url = "${pageContext.request.contextPath}/member.do";
		var args = {"username":username,"time":new Date(),"action":"seekAdministrator"};
		$.getJSON(url,args,function(data){
			if(jQuery.isEmptyObject(data)){
				$("#info").html("<font color='red'>此管理员不存在</font>");
				return ;
			}
			$("#thead").append("<tr><th>ID</th><th>用户名</th><th>密码</th><th>姓名</th><th>性别</th><th>出生日期</th>"
					+ "<th>身份证</th><th>手机号</th><th>所属组织</th><th>总权值</th>"
					+ "<th>更新管理员信息</th><th>删除管理员</th></tr>");
			show(data);
	...
	$("#btn2").click(function(){
		...
		$("#info").empty();
		var url = "${pageContext.request.contextPath}/member.do";
		var args = {"time":new Date(),"action":"seekAdministrators"};
		$.getJSON(url,args,function(data){
			if(jQuery.isEmptyObject(data)){
				$("#info").html("<font color='red'>没有管理员信息</font>");
				return ;
			}
			$("#thead").append("<tr><th>ID</th><th>用户名</th><th>密码</th><th>姓名</th><th>性别</th><th>出生日期</th>"
					+ "<th>身份证</th><th>手机号</th><th>所属组织</th><th>总权值</th>"
